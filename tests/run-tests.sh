...
cp "$ELM_FORMAT" tests/elm-format-short
ELM_FORMAT="tests/elm-format-short"
...
function checkWaysToRun() {
	...
	EMPTY_DIR=$(mktemp -d -t elm-format-short-tests.XXXXXXXXXX)
	...
	echo "## elm-format-short DIRECTORY --yes"
	"$ELM_FORMAT" "$DIRECTORY" --yes 1>/dev/null 2>/dev/null
	returnCodeShouldEqual 1

	echo "## elm-format-short EMPTY_DIRECTORY"
	"$ELM_FORMAT" "$EMPTY_DIR" 1>/dev/null
	returnCodeShouldEqual 1

	echo "## elm-format-short INPUT --validate"
	"$ELM_FORMAT" "$INPUT" --validate 1>/dev/null
	returnCodeShouldEqual 0

	echo "## elm-format-short INPUT --validate --yes"
...
echo
echo
echo "# elm-format-short test suite"
