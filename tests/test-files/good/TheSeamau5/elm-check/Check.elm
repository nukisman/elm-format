...
-}
type alias UnitEvidence = Result FailureOptions SuccessOptions
...
claim name actualStatement expectedStatement investigator =
  ...
  <|
    \numberOfChecks seed ->
      ...
      -- code.
      let -- Find the original counter example. The original counter example
            ...
            Err ( originalCounterExample, originalActual, originalExpected, seed, numberOfChecks ) ->
              let ------------------------------------------------------------------
                  ...
                  shrink counterExample currentNumberOfShrinks =
                    let -- Produce a list of values considered more minimal that
                  ...
                  -- minimal : a
                  -- numberOfShrinks : Int
                  ( minimal, numberOfShrinks ) = trampoline (shrink originalCounterExample 0)
                  -- actual : b
                  actual = actualStatement minimal
                  -- expected : b
                  expected = expectedStatement minimal
              in  -- Here, we return an `Err` signifying that a counter example was
...
claimTrue name predicate = claim name predicate (always True)
...
claimFalse name predicate = claim name predicate (always False)
...
perform 100 checks and use `Random.initialSeed 1` as the random seed.
    ...
    quickCheck claim = check claim 100 (Random.initialSeed 1)
...
suite name claims = Suite name claims
...
claim2 name actualStatement expectedStatement specA specB =
  claim name
    (\( a, b ) -> actualStatement a b)
    (\( a, b ) -> expectedStatement a b)
    (tuple ( specA, specB ))
...
claim3 name actualStatement expectedStatement specA specB specC =
  claim name
    (\( a, b, c ) -> actualStatement a b c)
    (\( a, b, c ) -> expectedStatement a b c)
    (tuple3 ( specA, specB, specC ))
...
claim4 name actualStatement expectedStatement specA specB specC specD =
  claim name
    (\( a, b, c, d ) -> actualStatement a b c d)
    (\( a, b, c, d ) -> expectedStatement a b c d)
    (tuple4 ( specA, specB, specC, specD ))
...
claim5 name actualStatement expectedStatement specA specB specC specD specE =
  claim name
    (\( a, b, c, d, e ) -> actualStatement a b c d e)
    (\( a, b, c, d, e ) -> expectedStatement a b c d e)
    (tuple5 ( specA, specB, specC, specD, specE ))
